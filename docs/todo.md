...
- [x] v11 lambda single-expression restriction removed: block-body lambda parsing + statement-level capture collect/rewrite + tail-expression auto-return + regression suite case (2026-03-02)
- [x] v11 safe-navigation SSA global fallback removed (`cg_program_with_sigs` no longer forces non-SSA on safe-nav presence) (2026-03-02)
- [x] v11 value-generic parser gate widened from `u64`-only to integer/bool allow-list (`parse_is_value_generic_type_allowed`) (2026-03-02)
- [x] v11 const-generic value literal path expanded: generic-call lookahead + generic arg parser now accept `true`/`false` and signed numeric literals (`-N`) with regression suite coverage (2026-03-02)
- [x] v11 throw outside try restriction removed: parser now lowers out-of-try `throw expr;` into explicit unhandled-throw fail path (2026-03-02)
- [x] v11 value-generic strict type compatibility check (arg literal vs declared const generic type kind) in semantic phase (2026-03-03)
- [x] v11 exception model phase-2: decouple parser-level try/catch/throw lowering into dedicated middle-end lowering pass (2026-03-02)
- [x] v11 exception phase-2 follow-up cleanup: remove parser legacy try-context loop-guard injection/state fields after middle-end lowering migration (2026-03-03)
- [x] v11 exception lowering internal cleanup: remove unused depth-tracking/dead helper path in compiler pass while preserving behavior (2026-03-03)
- [x] v11 exception lowering dedup cleanup: factor try/catch/finally lowered-wrap append path into shared helper (2026-03-03)
- [x] v11 exception lowering dedup cleanup: factor repeated TypeInfo -> AstVarDecl copy path for throw payload/catch binding declarations (2026-03-03)
- [x] v11 null typing semantics phase-1: introduce TYPE_NULL, parse `null` as typed literal cast, enforce `null -> pointer` assignability only, and add regression pass/fail cases (2026-03-03)
- [x] v11 throw payload typing stabilization phase-1: remove implicit `u64` payload fallback, require explicit typed catch binding, reuse outer payload slot for nested rethrow paths, and enforce compile-time throw payload compatibility checks with regression coverage (2026-03-03)
- [x] v11 optional-safe indexing (`?[idx]`) end-to-end: AST/Parser/Emitter/SSA integration, lvalue rejection, pointer-null guard + zero fallback semantics, and regression suite coverage (2026-03-02)
- [x] v11 try operator compatibility generalized from name-based Option/Result detection to structural check/value field resolution (`is_some/value`, `is_ok/ok`, `is_ok/value`) across compiler/emitter/SSA + regression coverage (2026-03-02)
- [x] v11 investigate parser regression around `CASE 154` (`const_flag<true>()` in core extension suite): not reproducible on current source after clean stage0/stage1 rebuild + filtered verification run (2026-03-03)
- [x] v11 fix runtime regression on `CASE 156` (`85_core_language_extensions_suite::156_try_structural_check_value_fields_success`): fixed packed small-struct return literal path (`cg_return_struct_small`) + try check-field width-aware load (`cg_try_emit_fail_return`), verified green on full profile (`579/579`) (2026-03-03)
- [x] v11 SSA try-compat alignment: `builder_try_resolve_meta_or_panic` switched from legacy kind-based Option/Result gating to structural field-based resolver (`compiler_try_field_names_from_typeinfo`) with unified diagnostics and full regression green (`579/579`) (2026-03-03)
- [x] v11 production fuzz harness folder (`fuzz/`) with corpus bootstrap, mutation/synthesis, parallel matrix differential checks, and finding triage output (2026-03-02)
- [x] v11 crash finding triage automation (`fuzz/triage_crash.sh`) + stage-hypothesis summary output (2026-03-02)
- [x] v11 fuzz→fix loop hardening: parser/compiler null-guard crash fixes + 500-iter verification campaign with crash=0/hang=0/diff=0 (2026-03-02)
- [x] v11 emitter crash hardening: `cg_expr`/`cg_lvalue` null-node guard clauses + aggressive fuzz recheck (20k iters, 32 jobs, 2s timeout) with crash=0 (2026-03-02)
- [x] v11 hang triage hardening: load-timeout 재현 분류 + `FUZZ_HANG_CONFIRM_SEC` 2차 확인 + `fuzz_norm_err` SIGPIPE(141) 제거 + 10k aggressive verify crash=0/hang=0/diff=0 (2026-03-03)
- [x] v11 fuzz CI nightly job + findings artifact retention policy (`.github/workflows/fuzz-nightly.yml`, retention 14d, 2026-03-02)
- [ ] v11 triage precision upgrade: pass-level breadcrumbs / parser-only canary path (todo)
...
- [x] v4 inline asm register alias (`alias rax: name;`) + SSA/AST + tests (2026-02-05)
- [x] v4 defer statement (AST codegen + break/continue/return) + tests (2026-02-05)
- [x] v4 defer unwind emission keeps stack (continue/break/return fix) (2026-02-05)
- [x] v4 defer function-call test coverage (2026-02-05)
...
- [x] v4 ssa codegen asm text_vec uses *Vec<u64> (remove u64 param) (2026-02-05)
- [x] v4 mem2reg child_arr cast to *Vec<*SSABlock> (explicit) (2026-02-05)
- [x] v4 regalloc color_fn uses Vec for used/colors (reduce pointer ops) (2026-02-05)
- [x] v4 regalloc color_fn returns *Vec<u64> (remove colors array pointer math) (2026-02-05)
- [x] v4 regalloc map/apply uses direct array access (remove helpers) (2026-02-05)
- [x] v4 regalloc manual pointer arithmetic replaced with [] indexing (2026-02-05)
- [x] v4 src manual pointer arithmetic replaced with [] indexing (2026-02-05)
- [x] v4 test add slice of pointer elements ([]*u8) (2026-02-05)
...
- [x] v4 SSA/Emitter/Builder 포인터 Vec 명시화 정리 + build_and_test 통과 (2026-02-03)
- [x] v4 builder_add_params params_vec 타입 명시화 + build_and_test 통과 (2026-02-03)
- [x] v4 AstFunc params_vec 포인터 Vec로 명시화 + build_and_test 통과 (2026-02-03)
- [x] v4 AST 벡터 필드 전반 *Vec 타입 명시화 + build_and_test 통과 (2026-02-03)
- [x] v4 타입/SSA 구조체 벡터 필드 추가 명시화 + build_and_test 통과 (2026-02-03)
- [x] v4 AST 생성자 포인터 리턴화 + 호출부 캐스팅 정리 + build_and_test 통과 (2026-02-03)
- [x] v4 AST u64 캐스팅 제거 (AstNode 포인터 통일) + build_and_test 통과 (2026-02-03)
- [x] v4 Vec/HashMap 핸들(u64) 제거 및 포인터 기반 전환 + build_and_test 통과 (2026-02-02)
- [x] v4 compiler.bpp 제네릭 컨테이너 타입 구조화 + 중첩 제네릭 구문 정정 + build_and_test 통과 (2026-02-02)
- [x] v4 파서: 타입/호출/파라미터 `>>` 인식 + parser.util types import/부트스트랩 안정화 + 테스트 통합 (2026-02-03)
- [x] v4 파서: parse_peek/parse_prev *Token 리턴화 + 전체 호출부 정리 + build_and_test 통과 (2026-02-03)
- [x] v4 파서/렉서: tok_new/parse_type_ex/parse_decl 리턴 타입 포인터화 + 호출부 정리 + build_and_test 통과 (2026-02-03)
- [x] v4 SSA: 컨텍스트/함수/블록 생성자 포인터 리턴 정리 + 코드젠 연동 타입 정리 + build_and_test 통과 (2026-02-03)
...
- [x] v1 컴파일러 보강: 함수 포인터/struct 반환/packed/large struct 안정화 + 전체 테스트 통과 (2026-02-01)
- [x] v1 제네릭 struct 지원 + 10_generics 통합 검증 (2026-02-01)
- [x] v2 std Vec/HashMap 제네릭 + impl 통합 (2026-02-01)
- [x] v2 impl 제네릭 이름 충돌 해결 (2026-02-01)
...
- [x] v2 std Vec elem stride를 sizeof(T) 기반으로 전환 (2026-02-02)
- [x] v2 2-pass 모듈 로딩(시그니처 선등록) 도입 (2026-02-02)
- [x] v2 2-pass import/generic 템플릿 테스트 추가 (2026-02-02)
- [x] v2 2-pass 안정성 테스트(순환/alias/체인) 추가 (2026-02-02)
- [x] v2 테스트 모듈 파일 modules/로 정리 (2026-02-02)
- [x] v2 Vec<T> struct 저장 통합 테스트 추가 (2026-02-02)
- [x] v2 Vec<T> struct 극한 케이스 테스트 추가 (2026-02-02)
- [x] v2 Pass1 import-only 선스캔으로 제네릭 템플릿 미등록 오류 방지 (2026-02-02)
- [x] v2 generic var decl TYPE_GENERIC 이름 보존 파서 수정 (2026-02-02)
- [x] v2 method call sret 처리 + Vec pop 대형 struct 안정화 (2026-02-02)
...
- [x] v3 hashmap_* 래퍼 제거 + HashMap 메서드 전환 + std hashmap 테스트 수정 (2026-02-02)
- [x] v3 전역 변수 타입 확장(u64 제한 해제) + 글로벌 타입 기반 BSS/코드젠 + 통합 테스트 추가 (2026-02-02)
