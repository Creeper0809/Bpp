...
- compiler-known prelude에는 `number`, `string`, typed `input()`, `print`/`println`, `emit`/`emitln`처럼 컴파일러가 특별 취급하는 이름이 포함됩니다.
...
- typed `input()` 같은 문맥 설탕은 기대 타입에 따라 lowering 경로가 달라집니다.
