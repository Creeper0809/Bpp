...
# Bootstrap workaround: base compiler may not parse '>>' in compiler.bpp yet.
TEMP_COMPILER="$SCRIPT_DIR/src/compiler.bpp"
TEMP_COMPILER_BAK="$SCRIPT_DIR/src/compiler.bpp.bak"
if [ -f "$TEMP_COMPILER" ]; then
    cp "$TEMP_COMPILER" "$TEMP_COMPILER_BAK"
    sed 's/>>/> >/g' "$TEMP_COMPILER_BAK" > "$TEMP_COMPILER"
fi
...
if [ -f "$TEMP_COMPILER_BAK" ]; then
    mv -f "$TEMP_COMPILER_BAK" "$TEMP_COMPILER"
fi
